...
def add_patches_to_img(ax, molx, moly):
    for i in range(0, molx.size):
        try:
            ...
            print("could not add the patches.")
            return
...
def plot_mols(data, n, molname):
    ...
    for i in range(0, n):
        plt.plot(data.Frequency / 1e9, (data['mol' + str(i)] + i * 10000), label='mol' + str(i))
        plt.show()
...
def plot_tera(new_data, reader, center=(5, 5), rngs=(3, 3), save_name="tera_scan"):
    ...
    fluor_trace = np.mean(np.mean(
        frames[:, center[0] - rngs[0] // 2:center[0] + rngs[0] // 2, center[1] - rngs[1] // 2:center[1] + rngs[1] // 2],
        axis=1), axis=1)
    data.c.append(fluor_trace,
                  names="Fluor")  # appends a new colum to data, new colum data is fluor_trace, name is "Fluor"
